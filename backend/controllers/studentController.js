...
exports.getStudentProfile = async (req, res) => {
    try {
        ...
        // Fetch certifications (NOT IMPLEMENTED YET, but in future, we can use it.)
...
//         // Custom File Name with student id and timestamp to avoid collisions
...
//                 resource_type: 'raw',     // Because a PDF is not an image.
...
//                 await db.query(
//                     // Now the database stores only the Cloudinary URL, not the file itself.
...
exports.deleteSkill = async (req, res) => {
    ...
    try {
        // Fetch student ID matching current authenticated user
        const [student] = await db.query('SELECT id FROM students WHERE user_id = ?', [req.user.id]);
